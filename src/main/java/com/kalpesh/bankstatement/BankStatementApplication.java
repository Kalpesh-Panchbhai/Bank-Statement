...
public class BankStatementApplication implements CommandLineRunner {
    ...
    public void run(String... args) throws IOException {
        ...
        for (int j = 0; j < contents.size(); j++) {
            if (contents.get(j).endsWith("_yearly.pdf")) {
                processYearlySheet(currentDir, contents, j);
            } else {
                processMonthlySheet(currentDir, contents, j);
            }
        }
    }

    private void processMonthlySheet(String currentDir, List<String> contents, int j) throws IOException {
        int startingIndex = 0;
    ...
    private void processYearlySheet(String currentDir, List<String> contents, int j) throws IOException {
        int startingIndex = 0;
        PDDocument document = PDDocument.load(new File(currentDir + "/" + contents.get(j)));
        if (document.isEncrypted()) {
            PDFTextStripper stripper = new PDFTextStripper();
